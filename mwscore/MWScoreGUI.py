...
class MWScoreFrame(wx.Frame):
    ...
    def __init__(self, initial_mechs=None):
        ...
        self.TransponderMenu.Append(
            self.ID_TRANSPONDERVAR, "Transponder Variable Setup"
        )
        self.TransponderMenu.Append(
            self.ID_TRANSPONDERHPUPDATE, "Refresh Transponder HP"
        )
    ...
    def MatchSetup(self, event):
        ...
        self.ScoreServer.Match = MWScore.Match(
            self.ScoreServer, MatchType, MatchLength, MatchRuleSet, MechList
        )
    ...
    def TransponderVar(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            self.ScoreServer.TransponderListener.WriteTransponderNewID(
                int(dlg.CurrentIDChoice.GetValue()), int(dlg.NewIDChoice.GetValue())
            )
            self.ScoreServer.TransponderListener.WriteTransponder(
                int(dlg.NewIDChoice.GetValue()),
                int(dlg.SetHpChoice.GetValue()),
                MatchRuleSet,
            )
            if not self.ScoreServer.TransponderListener.Xbee:
                wx.MessageBox(
                    "Could not open Transponder port " + dlg.PortChoice.GetValue(),
                    "Error",
                    wx.OK | wx.ICON_ERROR,
                )
        dlg.Destroy()
...
class MatchDialog(wx.Dialog):
    ...
    def __init__(self, parent, id, data):
        ...
        self.MatchLengthChoice = wx.ComboBox(
            self,
            -1,
        ...
        self.MatchTypeChoice = wx.ComboBox(
            self, -1, style=wx.CB_DROPDOWN, choices=typeChoices
        )
        ...
        self.NumTeamsChoice = wx.ComboBox(
            self,
            -1,
            style=wx.CB_DROPDOWN,
        ...
        self.MatchRulesChoice = wx.ComboBox(
            self, -1, style=wx.CB_DROPDOWN, choices=rulesChoices
        )
        if data.get("MatchRuleSet", None) is not None:
            self.MatchRulesChoice.SetValue(
                str(rulesChoices[data.get("MatchRuleSet", 0)])
            )
        ...
        MatchLengthSizer.Add(
            self.MatchLengthText, 1, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5
        )
        ...
        MatchRulesSizer.Add(
            self.MatchRulesText, 1, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5
        )
...
class SocketServerDialog(wx.Dialog):
    ...
    def __init__(self, parent, id):
        ...
        self.HostChoice = wx.TextCtrl(
            self, -1, str(parent.ScoreServer.SocketServer.Host)
        )

        self.PortText = wx.StaticText(self, -1, "Port:")
        self.PortChoice = wx.TextCtrl(
            self, -1, str(parent.ScoreServer.SocketServer.Port)
        )
...
class TransponderDialog(wx.Dialog):
    ...
    def __init__(self, parent, id):
        ...
        self.PortText = wx.StaticText(self, -1, "Port:")
        self.PortChoice = wx.TextCtrl(
            self, -1, str(parent.ScoreServer.TransponderListener.Port)
        )

        self.BaudText = wx.StaticText(self, -1, "Host:")
        self.BaudChoice = wx.TextCtrl(
            self, -1, str(parent.ScoreServer.TransponderListener.Baudrate)
        )
...
class MatchPanel(wx.Panel):
    ...
    def __init__(self, parent, id):
        ...
        for m in range(len(self.MechList)):
            ...
            self.MechHPText.append(
                MechHPText(self, -1, self.ScoreServer, self.MechList[m])
            )
        ...
        self.Sizer.Add(
            self.MatchTimerText, proportion=0, flag=wx.ALL | wx.ALIGN_CENTER, border=10
        )
        ...
        for m in range(len(self.MechList)):
            ...
            if self.Match.MatchType != MWScore.MATCH_FFA:
                if self.MechList[m].Team != self.MechList[m - 1].Team and m != 0:
                    ...
                    self.Sizer.Add(
                        vstext, proportion=0, flag=wx.ALL | wx.ALIGN_CENTER, border=10
                    )

            self.MechSizer[m].Add(
                self.MechNameText[m], proportion=0, flag=wx.RIGHT, border=10
            )
            self.MechSizer[m].Add(
                self.MechHPText[m], proportion=0, flag=wx.LEFT, border=10
            )
...
class MatchTimerText(wx.StaticText):
    ...
    def Refresh(self):
        ...
        t_sec = int((t_time - (t_min * 600)) * 0.1)
