...
class SocketServer(ScoreModule):
    ...
    def Run(self):
        while not self.ThreadKill:
            ...
            if self.Socket == None:
                self.ScoreServer.Log(
                    "No socket found in socket thread; exiting thread."
                )
                time.sleep(1)
                return
    ...
    def Broadcast(self, msg):
        for client in self.Clients:
            try:
                ...
                self.ScoreServer.Log(
                    "Disconnecting client " + repr(client.getpeername())
                )
...
class TransponderListener(ScoreModule):
    ...
    def Setup(self, port, baud):
        ...
        try:
            ...
            self.ScoreServer.Log(
                " TransponderListener setup succesful on port " + self.Port + " ! \r\n"
            )
            self.StartThread()
            return True
        except Exception as x:
            self.Xbee = None
            self.ScoreServer.Log(
                "Setup exception! TransponderListener not set up. Port "
                + self.Port
                + " error "
                + str(x)
                + "\r\n"
        ...
        try:
            while not self.ThreadKill:
                ...
                if (mechidh + mechidl) == 0xFF:
                    result = self.ScoreServer.MechList.MechByID(mechidh).AdjustHP(
                        mechhp
                    )
...
class Match(ScoreModule):
    ...
    def __init__(
        self, server, matchtype=MATCH_TEAM, matchlength=4800, matchrules=0, mechs=[]
    ):
        ScoreModule.__init__(self, server)
        ...
        if self.Time <= 0:
            ...
            if not self.SuddenDeath:
                ...
                if self.Teams[0].HP == self.Teams[1].HP:
                    ...
                    self.ScoreServer.Log(
                        "Match Time up... Tie found... SUDDEN DEATH MODE!"
                    )
    ...
    def MatchData(self):
        ...
        data = (
            str(self.Time) + ":" + str(self.MatchType) + ":" + str(len(self.MechList))
        )
